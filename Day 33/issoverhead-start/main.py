...
def _get_iss_coordinates() -> (float, float):
    response = requests.get(url="http://api.open-notify.org/iss-now.json")
    response.raise_for_status()

    iss_position = response.json()["iss_position"]
    iss_latitude, iss_longitude = iss_position["latitude"], iss_position["longitude"]
    return float(iss_latitude), float(iss_longitude)

def iss_is_overhead() -> bool:
    iss_latitude, iss_longitude = _get_iss_coordinates()
